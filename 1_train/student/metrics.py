...
def recall(matrix: tf.Tensor, idx: int) -> tf.Tensor:
    ...
    sumtrue=tf.reduce_sum(matrix[idx,:])
    truetrue=matrix[idx,idx]
    recall=tf.divide(truetrue,sumtrue)
...
def precision(matrix: tf.Tensor, idx: int) -> tf.Tensor:
    ...
    sumtrue=tf.reduce_sum(matrix[:,idx])
    truetrue=matrix[idx,idx]
    precision=tf.divide(truetrue,sumtrue)
...
def f1_score(matrix: tf.Tensor, idx: int) -> tf.Tensor:
    ...
    f1_score=tf.multiply(2,tf.multiply(recall_val,precision_val))
    f1_score=tf.divide(f1_score, tf.add(recall_val,precision_val))
    ### ENTER STUDENT CODE ABOVE ###
