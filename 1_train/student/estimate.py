...
def estimate_conv2d_macs(in_shape: List[int], kernel_shape: List[int], out_shape: List[int]):
    ...
    ### ENTER STUDENT CODE BELOW ###
    macs = kernel_h*kernel_w*output_h*output_w*input_c*output_c
...
def estimate_depthwise_conv2d_macs(
    ...
    ### ENTER STUDENT CODE BELOW ###
    macs = kernel_h*kernel_w*output_h*output_w*kernel_oc
...
def estimate_fully_connected_macs(in_shape: List[int], filter_shape: List[int], out_shape: List[int]):
    ...
    ### ENTER STUDENT CODE BELOW ###
    macs = filter_h*filter_w*output_w
...
def estimate_rom(tensors: List[MyTensor]):
    ...
    dtype_size = {"int8": 1, "int32": 4, "float32": 4}

    for tensor in tensors:
        if tensor.is_const:
            rom_bytes += reduce(operator.mul, tensor.shape, 1)*dtype_size[tensor.dtype]
...
def estimate_ram(tensors: List[MyTensor], layers: List[MyLayer]):
    ...
    for layer in layers:
        ...
        #print(layer.inputs)
        #print(layer.outputs)
        for tensor_i in layer.inputs + layer.outputs:
            tensor = tensors[tensor_i]
            if not tensor.is_const:
                #print("tensor.shape:", tensor.shape)
                size = reduce(operator.mul, tensor.shape, 1)*dtype_size[tensor.dtype]
                buffer_bytes += size
                #print("buffer_bytes:", buffer_bytes)
        if ram_bytes < buffer_bytes:
            ram_bytes = buffer_bytes
